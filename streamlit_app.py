...
# --- 2. GROUND TRUTH ENGINE (USGS Lucama 02090380) ---
def get_usgs_ground_truth():
    """Pulls 15-minute verification from USGS. Sensor trumps forecast."""
    try:
...
SOIL_MOISTURE_VAL = 0.058 # Restored constant
...
# --- 4. THE GROUND TRUTH OVERRIDE (CORE LOGIC) ---
# If USGS confirms 0.0" rain, override forecast for today and tomorrow
if usgs_rain == 0.0:
    for day in ["Wed", "Thu"]:
        if current_day == day:
...
# --- 5. UI RENDERING (ORIGINAL FORMATTING PRESERVED) ---
st.markdown(f"""
    <div class="exec-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div class="exec-title">Wayne Brothers</div>
            <div class="sync-badge">USGS TRUTH OVERRIDE ACTIVE • {current_time}</div>
        </div>
...
with c_main:
    ...
    st.markdown('<div class="directive-header">7-Day Weather Outlook (Forecast vs. Truth)</div>', unsafe_allow_html=True)
    f_cols = st.columns(7)
    for i, (day_key, d) in enumerate(tactical_map.items()):
        f_cols[i].markdown(f"""
            <div class="forecast-card" style="border-top: 4px solid {d['color']};">
                ...
                <div class="precip-box">{d['pop']}</div>
                <div style="font-size: 0.8em; color: #AAA;">{d['in']}</div>
            </div>
        """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

with c_metrics:
    # Analytical Metrics (USGS Rain + Restored Soil Moisture)
    st.markdown('<div class="report-section">', unsafe_allow_html=True)
    st.markdown('<div class="directive-header">Analytical Metrics</div>', unsafe_allow_html=True)
    st.metric("USGS 02090380 Rain", f"{usgs_rain}\"", delta="DRY (VERIFIED)" if usgs_rain == 0 else "RUNOFF RISK")
    st.metric("Soil Moisture (API)", SOIL_MOISTURE_VAL)
    st.metric("Basin SB3 Capacity", "58%", delta="STABLE" if usgs_rain == 0 else "CRITICAL", delta_color="normal" if usgs_rain == 0 else "inverse")
...
st.components.v1.html(f"""<iframe width="100%" height="450" src="https://embed.windy.com/embed2.html?lat=35.726&lon=-77.916&zoom=9&level=surface&overlay=radar" frameborder="0" style="border-radius:8px;"></iframe>""", height=460)import streamlit as st
import json
import pandas as pd
import datetime as dt
import requests
